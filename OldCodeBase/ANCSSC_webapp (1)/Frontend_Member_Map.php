...
$queryResult = mysqli_query($link, "SELECT ngo_name, lat, lng FROM users WHERE lat > 0 and lng > 0");
$queryData = [];
...
<body>
    ...
    <div class="main_title">MEMBER MAP</div>
    ...
    <script type="text/javascript">
        ...
        var members = <?php echo $usableData ?>;

        members.forEach(addToMap);

        function addToMap(member) {
            var coord = [member.lat, member.lng];
            L.marker(coord).addTo(member_map).bindPopup(member.ngo_name);
        }
    </script>
